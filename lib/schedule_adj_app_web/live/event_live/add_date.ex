defmodule ScheduleAdjAppWeb.EventLive.AddDate do
  ...
  def handle_params(_params, _uri, socket) do
    socket =
      socket
      # 空のadd_dateリストをソケットに用意
      |> assign(:add_date, [])
      # 空のadd_datetimeリストをソケットに用意
      |> assign(:add_datetime, [])
  ...
  def handle_event("delete_date", %{"date" => params_date} = params, socket) do
    # 日付を消す
    new_add_date =
      socket.assigns.add_date -- [params_date]

    # 時間を消す
    # 条件式に当てはまらないものだけを残す関数
    new_add_datetime =
      Enum.reject(socket.assigns.add_datetime, fn %{"date" => x} -> x == params_date end)
