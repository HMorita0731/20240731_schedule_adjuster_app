...
# イベント作成画面/機能
  ...
  def handle_event("next_page",%{"input_form" => params},socket) do
    ...
    if event_changeset.valid? && user_changeset.valid? do
      socket =
      socket
       |>assign(trigger_submit: true) #2つのcsをセッションに保存

       {:noreply, socket}
    else
      return_form = to_form(event_changeset, as: "input_form")#どっちのエラーの情報も持ったフォーム構造体を作りたい
      return_form = #error情報同士ををくっつける
        %{return_form | errors: return_form.errors ++ return_form.source.errors ++ user_changeset.errors}
      socket=
      socket
